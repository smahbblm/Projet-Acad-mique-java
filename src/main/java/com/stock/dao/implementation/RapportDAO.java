...
public class RapportDAO implements IRapportDAO {
    ...
    @Override
    public void create(Rapport rapport) throws Exception { // adaptation signature interface
        String sql = "INSERT INTO rapports (type, dateGeneration, dateDe, dateA, contenu) VALUES (?, ?, ?, ?, ?)";
        try (PreparedStatement ps = getConn().prepareStatement(sql, java.sql.Statement.RETURN_GENERATED_KEYS)) {
            ps.setString(1, rapport.getType());
            ps.setObject(2, rapport.getDateGeneration());
            ps.setObject(3, rapport.getDateDe());
            ps.setObject(4, rapport.getDateA());
            ps.setString(5, rapport.getContenu());
            ps.executeUpdate();
            try (ResultSet rs = ps.getGeneratedKeys()) { if (rs.next()) rapport.setIdRapport(rs.getInt(1)); }
        }
    }
